...
const DAYPASS_RESORT_TOUR_KEYS = [
...
const DAYPASS_EXCURSION_TOUR_KEYS = [
  "scape_park",
...
const ACTIVITIES_TOUR_KEYS = [
  "coco_bongo",
  "dinner_in_the_sky",
  "boat_party",
];

const HOTEL_SCOPE_OPTIONS = [
  { key: "nacionales", title: "Nacionales" },
  { key: "internacionales", title: "Internacionales" },
];
...
const HOTEL_ALL_INCLUSIVE_PROFILE_OPTIONS = [
  { key: "familiares", title: "Familiares 👪" },
  { key: "solo_adultos", title: "Solo adultos 🔞" },
  { key: "parque_acuatico", title: "Parque acuático 💦" },
];

const HOTEL_VIEW_MODE_OPTIONS = [
  { key: "por_polo", title: "Por polo turístico" },
  { key: "por_presupuesto", title: "Por presupuesto" },
];

const HOTEL_POLO_OPTIONS = {
  familiares: [
    { key: "boca_chica_juan_dolio", title: "Boca Chica - Juan Dolio" },
    { key: "la_romana", title: "La Romana" },
    { key: "bavaro_punta_cana", title: "Bávaro - Punta Cana" },
    { key: "miches", title: "Miches" },
    { key: "samana", title: "Samaná" },
    { key: "puerto_plata", title: "Puerto Plata" },
  ],
  solo_adultos: [
    { key: "la_romana", title: "La Romana" },
    { key: "bavaro_punta_cana", title: "Bávaro - Punta Cana" },
  ...
  parque_acuatico: [
    { key: "para_ninos", title: "Para niños" },
    { key: "para_grandes_y_chicos", title: "Para grandes y chicos" },
  ],
};

const HOTEL_BUDGET_OPTIONS = {
  familiares: [
    { key: "menos_30000", title: "Menos de RD$30,000" },
    { key: "30000_40000", title: "RD$30,000 - RD$40,000" },
    { key: "40000_50000", title: "RD$40,000 - RD$50,000" },
    { key: "50000_60000", title: "RD$50,000 - RD$60,000" },
    { key: "mas_60000", title: "Más de RD$60,000" },
  ],
  solo_adultos: [
    { key: "30000_40000", title: "RD$30,000 - RD$40,000" },
    { key: "40000_50000", title: "RD$40,000 - RD$50,000" },
  ...
  parque_acuatico: [
    { key: "menos_40000", title: "Menos de RD$40,000" },
    { key: "40000_50000", title: "RD$40,000 - RD$50,000" },
    { key: "50000_60000", title: "RD$50,000 - RD$60,000" },
    { key: "mas_60000", title: "Más de RD$60,000" },
  ],
...
const CRUISE_FERRY_OPTIONS = [
  { key: "ferries_caribe", title: "Ferries del Caribe" },
  { key: "desde_republica_dominicana", title: "Desde República Dominicana" },
  { key: "internacionales", title: "Internacionales" },
];
...
function quickHelpText() {
  return (
    `¡Hola! 😊\n` +
    `Puedo ayudarte con *Hoteles nacionales e internacionales*, *Paquetes internacionales*, *Cruceros y Ferries del Caribe*, *Promociones*, *Daypass excursiones*, *Daypass resorts*, *Entradas parques temáticos y actividades*, *Seguros de viajes*, *Traslados* y *Boletos aéreos*.\n\n` +
    `También puedes escribirme *"menú"* para ver todas las opciones disponibles.`
  );
}
...
function serviceLineRowTitle(service) {
  const map = {
    ...
    paquetes_vacacionales: "Paquetes intl.",
    cruceros_ferries: "Cruceros/Ferries",
    promociones: "Promociones",
    daypass_excursiones: "Daypass excursiones",
    daypass_resorts: "Daypass resorts",
    parques_actividades: "Parques/actividades",
    tours_colombia: "Actividades",
    boletos_aereos: "Boletos aéreos",
    seguros_viaje: "Seguros de viajes",
    traslados: "Traslados",
    cruceros: "Cruceros",
...
function detectServiceLineFromUser(text) {
  ...
  if (
    t.includes("hotel") || t.includes("hoteles") || t.includes("hospedaje") ||
    t.includes("alojamiento") || t.includes("todo incluido") || t.includes("solo alojamiento")
  ) return "solo_hoteles";

  if (
    t.includes("paquete internacional") || t.includes("paquetes internacionales") ||
    t.includes("sin visa") || t.includes("visa americana") || t.includes("otros visados")
  ) return "paquetes_vacacionales";

  if (t.includes("promocion") || t.includes("promociones") || t.includes("promo")) return "promociones";

  if (
    t.includes("crucero") || t.includes("cruceros") || t.includes("ferry") ||
    t.includes("ferries del caribe") || t.includes("royal caribbean") ||
    t.includes("msc") || t.includes("norwegian") || t.includes("costa cruise")
  ) return "cruceros_ferries";

  if (
    t.includes("daypass resort") || t.includes("daypass resorts") ||
    t.includes("daypass hotel") || t.includes("daypass hoteles") ||
    t.includes("resort daypass")
  ) return "daypass_resorts";

  if (
    t.includes("daypass excursion") || t.includes("daypass excursiones")
  ) return "daypass_excursiones";

  if (
    t.includes("parque tematico") || t.includes("parques tematicos") ||
    t.includes("parque temático") || t.includes("parques temáticos") ||
    t.includes("actividad") || t.includes("actividades") ||
    ...
    t.includes("hacienda park") || t.includes("boat party")
  ) return "parques_actividades";

  if (
    t.includes("whala") || t.includes("juan dolio") || t.includes("boca chica") ||
    t.includes("dreams dominicus") || t.includes("royalton") || t.includes("nickelodeon")
  ) return "daypass_resorts";

  if (
    t.includes("boleto") || t.includes("vuelos") || t.includes("vuelo") ||
...
function parseMenuOption(userText, options = []) {
  ...
  for (const option of options) {
    const titleNorm = normalizeText(option?.title || option?.label || option?.key || "");
    const keyNorm = normalizeText(String(option?.key || "").replace(/_/g, " "));
    if (normalized === titleNorm || normalized === keyNorm || normalized.includes(titleNorm) || normalized.includes(keyNorm)) {
      return option;
    }
  }
...
function formatNumberedOptionsText(title, intro, options = []) {
  return (
    `${title}\n\n` +
    `${intro}\n\n` +
    options.map((item, idx) => `${idx + 1}. ${item.title}`).join("\n") +
    `\n\nResponde con el *número* o el *nombre* de la opción.`
  );
}

function getHotelProfileOptions(profileTitle = "", modeTitle = "") {
  const profileKey = normalizeText(profileTitle).replace(/[^a-z0-9 ]/g, "").replace(/\s+/g, "_");
  const modeKey = normalizeText(modeTitle).includes("presupuesto") ? "por_presupuesto" : "por_polo";
  if (modeKey === "por_presupuesto") {
    if (profileKey.includes("solo_adultos") || profileKey.includes("solo adultos")) return HOTEL_BUDGET_OPTIONS.solo_adultos || [];
    if (profileKey.includes("parque_acuatico") || profileKey.includes("parque acuatico")) return HOTEL_BUDGET_OPTIONS.parque_acuatico || [];
    return HOTEL_BUDGET_OPTIONS.familiares || [];
  }

  if (profileKey.includes("solo_adultos") || profileKey.includes("solo adultos")) return HOTEL_POLO_OPTIONS.solo_adultos || [];
  if (profileKey.includes("parque_acuatico") || profileKey.includes("parque acuatico")) return HOTEL_POLO_OPTIONS.parque_acuatico || [];
  return HOTEL_POLO_OPTIONS.familiares || [];
}

function hotelNeedsBudgetPrompt(session) {
...
function buildHotelLeadSummary(session, phoneDigits) {
  const fields = [
    ...
    { label: "🌍 Alcance", value: session.pendingCategory || "—" },
  ];
...
async function finalizeHotelLead({ session, from }) {
  const phoneDigits = String(from || "").replace(/[^\d]/g, "");
  const summaryText = buildHotelLeadSummary(session, phoneDigits);
...
function buildCruiseFerryLeadSummary(session, phoneDigits) {
  const fields = [
    { label: "🧩 Servicio", value: serviceLineLabel("cruceros_ferries") },
    { label: "🛳️ Tipo", value: session.pendingCategory || "—" },
    { label: "📍 Ruta / destino", value: session.pendingDestination || "—" },
    { label: "📅 Fecha deseada", value: session.pendingTravelDateText || "—" },
    { label: "👥 Pasajeros", value: `${Number(session.pendingAdults || 0) + Number(session.pendingChildren || 0)} (${session.pendingAdults || 0} adultos / ${session.pendingChildren || 0} niños)` },
  ];
  if (Number(session.pendingChildren || 0) > 0) fields.push({ label: "👶 Edades de niños", value: session.pendingChildrenAges || "No especificadas" });
  fields.push({ label: "💵 Presupuesto", value: session.pendingRange || "—" });
  fields.push({ label: "👤 Cliente", value: session.pendingName || "—" });
  fields.push({ label: "📱 WhatsApp", value: phoneDigits || "—" });
  return buildLeadSummary("Nueva solicitud de cruceros y ferries del Caribe", fields);
}

async function finalizeCruiseFerryLead({ session, from }) {
  const phoneDigits = String(from || "").replace(/[^\d]/g, "");
  const summaryText = buildCruiseFerryLeadSummary(session, phoneDigits);
  updateLead(session, { tour_key: "", quotePreview: summaryText, converted: false, followupSent: false });
  await handoffToHumanTool({ summary: summaryText });
  await notifyPersonalWhatsAppLeadSummary(summaryText, phoneDigits);
  ...
  await sendWhatsAppText(
    from,
    `✅ *Solicitud recibida*\n\n` +
      `🛳️ Tipo: *${session.pendingCategory || "—"}*\n` +
...
async function startSimpleServiceFlow({ session, serviceLineKey, from }) {
  if (serviceLineKey === "paquetes_vacacionales") {
    ...
    session.packageMenuMode = "main";
    await sendWhatsAppText(from, `Perfecto 🌍 Vamos con *paquetes internacionales*.

Selecciona una opción principal para continuar con la cotización.`);
    await sendPackageDestinationsList(from, session);
    return true;
  }
...
Aquí te muestro las promociones disponibles para que elijas la que deseas consultar.`);
  ...
  if (serviceLineKey === "cruceros_ferries") {
  ...
  if (serviceLineKey === "parques_actividades") {
...
Ahora dime tu *nombre completo*.` },
  ...
  },
  {
    ...
    startPrompt: `Perfecto 🌍 Vamos con *paquetes internacionales*.

Responde con el *número* o el *nombre* del paquete que te interesa.`,
    afterStart: async ({ from }) => {
      await sendPackageDestinationsList(from, session);
    },
    summaryTitle: "Nueva solicitud de paquetes internacionales",
    buildSummaryFields: (session, phoneDigits) => [
...
Recibí tu solicitud de *paquetes internacionales* y nuestro equipo te contactará con opciones según:
` +
      `• destino: ${session.pendingDestination || "—"}
` +
...
` +
    `Te mostraré el listado disponible y puedes responder con el *número* o con el *nombre* de la opción que deseas ver.`
  );
}

function mainMenuText() {
  const visibleLines = SERVICE_LINES.filter((s) => s.visible !== false);
...
function getRealToursForContext(groupKey) {
  const keySet = new Set();
  if (groupKey === "daypass_resorts") {
    DAYPASS_RESORT_TOUR_KEYS.forEach((key) => keySet.add(key));
  } else if (groupKey === "daypass_excursiones") {
    DAYPASS_EXCURSION_TOUR_KEYS.forEach((key) => keySet.add(key));
  } else if (groupKey === "parques_tematicos_rd") {
    PARKS_THEME_TOUR_KEYS.forEach((key) => keySet.add(key));
  } else if (groupKey === "actividades_rd") {
    ACTIVITIES_TOUR_KEYS.forEach((key) => keySet.add(key));
  } else if (groupKey === "parques_actividades") {
    [...PARKS_THEME_TOUR_KEYS, ...ACTIVITIES_TOUR_KEYS].forEach((key) => keySet.add(key));
  } else {
    return getRealToursByGroup(groupKey || "tours_colombia");
  }

  const orderedKeys = Array.from(keySet);
  return orderedKeys.map((key) => getRealTourByKey(key)).filter(Boolean);
}

function getRealTourGroupIntro(groupKey) {
  if (groupKey === "daypass_resorts") return "Estas son las opciones de daypass resorts disponibles con Terramarine.";
  if (groupKey === "daypass_excursiones") return "Estas son las opciones de daypass excursiones disponibles con Terramarine.";
  if (groupKey === "parques_tematicos_rd") return "Estas son las opciones de parques temáticos disponibles con Terramarine.";
  if (groupKey === "actividades_rd") return "Estas son las actividades disponibles con Terramarine.";
  if (groupKey === "parques_actividades") return "Estas son las entradas y actividades disponibles con Terramarine.";
...
function getRealTourListTitle(groupKey) {
  if (groupKey === "daypass_resorts") return "🏖️ *Daypass resorts*";
  if (groupKey === "daypass_excursiones") return "🌴 *Daypass excursiones*";
  if (groupKey === "parques_tematicos_rd") return "🎟️ *Parques temáticos 🇩🇴*";
  if (groupKey === "actividades_rd") return "🌴 *Actividades 🇩🇴*";
  if (groupKey === "parques_actividades") return "🎟️ *Entradas parques temáticos y actividades 🇩🇴*";
  return "🎢 *Actividades y parques temáticos*";
}

function formatRealToursTextList(groupKey, session) {
  const tours = getRealToursForContext(groupKey);
  if (!tours.length) return "No encontré opciones disponibles ahora mismo 🙏";
  if (session) session.lastRealTours = tours.map((t) => ({ key: t.key, title: t.title }));
  return (
    `${getRealTourListTitle(groupKey)}\n\n` +
...
function buildRealTourLeadSummary(session, phoneDigits) {
  ...
  const fields = [
    { label: "🧩 Servicio", value: serviceLineLabel(session.pendingServiceLine || "tours_colombia") },
...
function buildPackageLeadSummary(session, phoneDigits) {
  ...
  const serviceTitle = serviceLineLabel(session.pendingServiceLine || "paquetes_vacacionales");
  const fields = [
    { label: "🧩 Servicio", value: serviceTitle },
    { label: "🌍 Opción", value: pkg?.title || session.pendingDestination || "—" },
  ];
  if (String(session.pendingNotes || "").trim()) fields.push({ label: "📌 Destino de interés", value: session.pendingNotes });
  fields.push({ label: "📅 Fecha de interés", value: session.pendingTravelDateText || "—" });
  fields.push({ label: "👥 Pasajeros", value: `${Number(session.pendingAdults || 0) + Number(session.pendingChildren || 0)} (${session.pendingAdults || 0} adultos / ${session.pendingChildren || 0} niños)` });
  if (Number(session.pendingChildren || 0) > 0) fields.push({ label: "👶 Edades de niños", value: session.pendingChildrenAges || "No especificadas" });
  fields.push({ label: "💵 Presupuesto", value: session.pendingRange || "—" });
  fields.push({ label: "👤 Cliente", value: session.pendingName || "—" });
  ...
  return buildLeadSummary(`Nueva solicitud de ${serviceTitle.toLowerCase()}`, fields);
}
...
async function finalizePackageLead({ session, from }) {
  ...
  const serviceTitle = serviceLineLabel(session.pendingServiceLine || "paquetes_vacacionales");
  const agesLine = Number(session.pendingChildren || 0) > 0 ? `\n👶 Edades: ${session.pendingChildrenAges || "No especificadas"}` : "";
  const interestLine = String(session.pendingNotes || "").trim() ? `\n📌 Destino de interés: *${session.pendingNotes}*` : "";
  await sendWhatsAppText(
    from,
    `✅ *Solicitud recibida*\n\n` +
      `🧩 Servicio: *${serviceTitle}*\n` +
      `🌍 Opción: *${pkg?.title || session.pendingDestination || "—"}*${interestLine}\n` +
      `📅 Fecha deseada: *${session.pendingTravelDateText || "—"}*\n` +
      `👥 Pasajeros: *${Number(session.pendingAdults || 0) + Number(session.pendingChildren || 0)}* (${session.pendingAdults || 0} adultos / ${session.pendingChildren || 0} niños)${agesLine}\n` +
      `💵 Presupuesto: *${session.pendingRange || "—"}*\n` +
      `👤 Nombre: *${session.pendingName || "—"}*\n\n` +
      `Te estaremos comunicando con un asesor para continuar con la búsqueda y validar disponibilidad.`
  );
  clearIntakeFlow(session);
}
...
function formatPackageDestinationsTextList(session) {
  ...
  return (
    ...
    `\n\nResponde con el *número* o con el *nombre* de la opción que deseas consultar.`
  );
}
...
async function sendCatalogDocument(to) {
  if (!CATALOG_DOCUMENT_URL) {
    await sendWhatsAppText(
      to,
      `Todavía no tengo el documento cargado aquí 🙏\n\nMientras tanto, dime si buscas *Hoteles nacionales e internacionales*, *Paquetes internacionales*, *Cruceros y Ferries del Caribe*, *Promociones*, *Daypass excursiones*, *Daypass resorts*, *Entradas parques temáticos y actividades*, *Seguros de viajes*, *Traslados* o *Boletos aéreos* y te ayudo por el flujo normal.`
    );
    return;
  }
...
- Hoteles nacionales e internacionales
- Paquetes internacionales
- Cruceros y Ferries del Caribe
- Promociones
- Daypass excursiones
- Daypass resorts
- Entradas parques temáticos y actividades
- Seguros de viajes
- Traslados
- Boletos aéreos
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    const hasEarlyIntent =
      ...
      tNorm.includes("actividad") ||
      tNorm.includes("parque") ||
      tNorm.includes("crucero") ||
    ...
    if (isGoBack(tNorm)) {
      ...
      if (realTourIntakeStates.includes(session.state)) {
        ...
        await sendWhatsAppText(from, `↩️ Perfecto. Volviste al listado anterior.`);
        await sendRealToursByGroup(from, session.pendingRealTourGroup || "tours_colombia", session);
        return res.sendStatus(200);
      }
      ...
      if (session.state === "await_tour_group" || ["tours_colombia", "daypass_excursiones", "daypass_resorts", "parques_actividades"].includes(session.pendingServiceLine)) {
        clearIntakeFlow(session);
        armMenuInactivityReminder(session);
        await sendWhatsAppText(from, mainMenuText());
        await sendServiceLinesList(from);
      ...
      if (["await_package_destination", "await_package_interest", "await_package_date", "await_package_adults", "await_package_children", "await_package_children_ages", "await_package_budget", "await_package_name"].includes(session.state)) {
        ...
        session.state = "await_package_destination";
        session.packageMenuMode = wasPromos ? "promos" : "main";
        await sendWhatsAppText(from, `↩️ Perfecto. Volviste al menú principal de *${wasPromos ? "promociones" : "paquetes internacionales"}*.`);
      ...
      if (["await_cruise_ferry_type", "await_cruise_ferry_route", "await_cruise_ferry_date", "await_cruise_ferry_adults", "await_cruise_ferry_children", "await_cruise_ferry_children_ages", "await_cruise_ferry_budget", "await_cruise_ferry_name"].includes(session.state)) {
        clearIntakeFlow(session);
        session.pendingServiceLine = "cruceros_ferries";
        session.state = "await_cruise_ferry_type";
        await sendWhatsAppText(from, `↩️ Perfecto. Volviste al menú principal de *cruceros y ferries del Caribe*.`);
        await sendWhatsAppText(from, formatNumberedOptionsText("🛳️ *Cruceros y Ferries del Caribe*", "Selecciona una opción:", CRUISE_FERRY_OPTIONS));
        return res.sendStatus(200);
      }

      if (session.state === "await_parks_activities_type") {
        clearIntakeFlow(session);
        session.pendingServiceLine = "parques_actividades";
        session.state = "await_parks_activities_type";
...
Responde con el *número* o con el *nombre* exacto de la opción.`
      ...
      session.pendingRealTourGroup = session.pendingRealTourGroup || pickedTour.groupKey;
      updateLead(session, {
...
👶 Edades: ${session.pendingChildrenAges}` : "";
      const realTourServiceTitle = serviceLineLabel(session.pendingServiceLine || "tours_colombia");
      await sendWhatsAppText(
        from,
        `✅ *Solicitud recibida*

` +
          `🧩 Servicio: *${realTourServiceTitle}*
` +
          `🎢 Opción: *${tour?.title || "—"}*
` +
          `📅 Fecha solicitada: *${dateMeta.display || session.pendingDesiredDate || "—"}*
...
` +
    ...
    if (session.state === "await_hotel_scope_choice") {
      const option = parseMenuOption(userText, HOTEL_SCOPE_OPTIONS);
      if (!option) {
        await sendWhatsAppText(from, formatNumberedOptionsText("🏨 *Hoteles nacionales e internacionales*", "Selecciona una opción:", HOTEL_SCOPE_OPTIONS));
        return res.sendStatus(200);
      }
    ...
    if (session.state === "await_hotel_national_type") {
      const option = parseMenuOption(userText, HOTEL_NATIONAL_STAY_OPTIONS);
      if (!option) {
        await sendWhatsAppText(from, formatNumberedOptionsText("🇩🇴 *Hoteles nacionales*", "Selecciona el tipo de estadía que deseas:", HOTEL_NATIONAL_STAY_OPTIONS));
        return res.sendStatus(200);
      }
    ...
    if (session.state === "await_hotel_profile") {
      ...
      session.state = "await_hotel_view_mode";
      await sendWhatsAppText(from, formatNumberedOptionsText("📍 *¿Cómo deseas ver las opciones?*", "Selecciona una opción:", HOTEL_VIEW_MODE_OPTIONS));
      return res.sendStatus(200);
    }

    if (session.state === "await_hotel_view_mode") {
      const option = parseMenuOption(userText, HOTEL_VIEW_MODE_OPTIONS);
      if (!option) {
        await sendWhatsAppText(from, formatNumberedOptionsText("📍 *¿Cómo deseas ver las opciones?*", "Selecciona una opción:", HOTEL_VIEW_MODE_OPTIONS));
        return res.sendStatus(200);
      }
    ...
    if (session.state === "await_hotel_adults_custom") {
      const count = parsePassengerCount(userText);
      if (count === null || count < 1) {
      ...
      session.pendingAdults = count;
      session.pendingPassengers = count;
      session.state = "await_hotel_children_custom";
      await sendWhatsAppText(from, `Gracias. Ahora dime cuántos *niños* viajarían. Si no van niños, responde *0*.`);
      return res.sendStatus(200);
    }

    if (session.state === "await_hotel_children_custom") {
      const count = parsePassengerCount(userText);
      if (count === null || count < 0) {
        await sendWhatsAppText(from, `Indícame cuántos *niños* viajarían. Si no van niños, responde *0*.`);
        return res.sendStatus(200);
      ...
      if (hotelNeedsBudgetPrompt(session)) {
        session.state = "await_hotel_budget_custom";
    ...
    if (session.state === "await_hotel_budget_custom") {
      if (tNorm.length < 2) {
    ...
    if (session.state === "await_hotel_name_custom") {
      if (tNorm.length < 3 || ["si", "sí", "ok", "listo"].includes(tNorm)) {
        await sendWhatsAppText(from, `Por favor, envíame tu *nombre completo* 🙂`);
        return res.sendStatus(200);
      }
      session.pendingName = userText;
      await finalizeHotelLead({ session, from });
      return res.sendStatus(200);
    }

    // =========================
    // CUSTOM CRUISES / FERRIES FLOW
    // =========================
    if (session.state === "await_cruise_ferry_type") {
      const option = parseMenuOption(userText, CRUISE_FERRY_OPTIONS);
      if (!option) {
        await sendWhatsAppText(from, formatNumberedOptionsText("🛳️ *Cruceros y Ferries del Caribe*", "Selecciona una opción:", CRUISE_FERRY_OPTIONS));
        return res.sendStatus(200);
      }
      session.pendingCategory = option.title;
      session.state = "await_cruise_ferry_route";
      await sendWhatsAppText(from, `Perfecto 🙌\n\nAhora indícame la *ruta o destino de interés*.`);
      return res.sendStatus(200);
    }

    if (session.state === "await_cruise_ferry_route") {
      if (tNorm.length < 2) {
        await sendWhatsAppText(from, `Por favor, indícame la *ruta o destino de interés*.`);
        return res.sendStatus(200);
      }
      session.pendingDestination = userText;
      session.state = "await_cruise_ferry_date";
      await sendWhatsAppText(from, `Perfecto 👍\nAhora indícame la *fecha deseada*.`);
      return res.sendStatus(200);
    }

    if (session.state === "await_cruise_ferry_date") {
      if (tNorm.length < 2) {
        await sendWhatsAppText(from, `Por favor, indícame la *fecha deseada*.`);
        return res.sendStatus(200);
      }
      session.pendingTravelDateText = userText;
      session.state = "await_cruise_ferry_adults";
      await sendWhatsAppText(from, `Perfecto 👍\n¿Cuántos *adultos* viajarían?`);
      return res.sendStatus(200);
    }

    if (session.state === "await_cruise_ferry_adults") {
      const count = parsePassengerCount(userText);
      if (count === null || count < 1) {
        await sendWhatsAppText(from, `Indícame cuántos *adultos* viajarían. Ej: 2`);
        return res.sendStatus(200);
      ...
      session.pendingAdults = count;
      session.pendingPassengers = count;
      session.state = "await_cruise_ferry_children";
      await sendWhatsAppText(from, `Gracias. Ahora dime cuántos *niños* viajarían. Si no van niños, responde *0*.`);
      return res.sendStatus(200);
    }

    if (session.state === "await_cruise_ferry_children") {
      const count = parsePassengerCount(userText);
      if (count === null || count < 0) {
        await sendWhatsAppText(from, `Indícame cuántos *niños* viajarían. Si no van niños, responde *0*.`);
        return res.sendStatus(200);
      ...
      session.pendingPassengers = Number(session.pendingAdults || 0) + count;
      if (count > 0) {
        session.state = "await_cruise_ferry_children_ages";
        await sendWhatsAppText(from, count > 1 ? `Perfecto. Ahora indícame las *edades de los niños separadas por coma*.\nEj: 5, 8` : `Perfecto. Ahora indícame la *edad del niño*.`);
        return res.sendStatus(200);
      }
      session.pendingChildrenAges = "";
      session.state = "await_cruise_ferry_budget";
      await sendWhatsAppText(from, `Ahora indícame tu *presupuesto aproximado* para ayudarte mejor con la búsqueda.`);
      return res.sendStatus(200);
    }

    if (session.state === "await_cruise_ferry_children_ages") {
      const ages = normalizeChildrenAgesInput(userText, session.pendingChildren || 0);
      if (!ages.ok) {
        await sendWhatsAppText(from, `Por favor, envíame las *edades de los niños separadas por coma*.\nEj: 5, 8`);
        return res.sendStatus(200);
    ...
    if (session.state === "await_cruise_ferry_budget") {
      if (tNorm.length < 2) {
        await sendWhatsAppText(from, `Por favor, indícame tu *presupuesto aproximado*.`);
        return res.sendStatus(200);
      }
      session.pendingRange = userText;
      session.state = "await_cruise_ferry_name";
      await sendWhatsAppText(from, `Perfecto ✅\nAhora indícame tu *nombre completo*.`);
      return res.sendStatus(200);
    }

    if (session.state === "await_cruise_ferry_name") {
      if (tNorm.length < 3 || ["si", "sí", "ok", "listo"].includes(tNorm)) {
        await sendWhatsAppText(from, `Por favor, envíame tu *nombre completo* 🙂`);
        return res.sendStatus(200);
      }
      session.pendingName = userText;
      await finalizeCruiseFerryLead({ session, from });
      return res.sendStatus(200);
    }

    if (session.state === "await_parks_activities_type") {
      const option = parseMenuOption(userText, PARKS_ACTIVITIES_OPTIONS);
      if (!option) {
        await sendWhatsAppText(from, formatNumberedOptionsText("🎟️ *Entradas parques temáticos y actividades 🇩🇴*", "Selecciona una opción:", PARKS_ACTIVITIES_OPTIONS));
        return res.sendStatus(200);
      }
...
Aquí tienes las opciones disponibles.`);
    ...
    if (directRealTourKey) {
      ...
      clearIntakeFlow(session);
      disableMenuInactivityReminder(session);
      session.pendingServiceLine = DAYPASS_RESORT_TOUR_KEYS.includes(directRealTourKey)
        ? "daypass_resorts"
        : DAYPASS_EXCURSION_TOUR_KEYS.includes(directRealTourKey)
          ? "daypass_excursiones"
          : "parques_actividades";
      await routeConversationToQueue({ session, phone: from, queueName: HUB_QUEUE_SERVICE_TOURS, reason: session.pendingServiceLine || "tours_colombia" });
      session.pendingRealTourGroup = DAYPASS_RESORT_TOUR_KEYS.includes(directRealTourKey)
        ? "daypass_resorts"
        : DAYPASS_EXCURSION_TOUR_KEYS.includes(directRealTourKey)
          ? "daypass_excursiones"
          : tour?.groupKey || "parques_actividades";
    ...
    if (serviceLineKey) {
      ...
      if (["tours_colombia", "parques_actividades", "daypass_excursiones", "daypass_resorts", "boletos_aereos", "solo_hoteles", "cruceros", "ferries_caribe", "cruceros_ferries", "seguros_viaje", "traslados", "hablar_asesor"].includes(serviceLineKey)) {
        await routeConversationToQueue({ session, phone: from, queueName: HUB_QUEUE_SERVICE_TOURS, reason: serviceLineKey });
      } else if (["paquetes_vacacionales", "promociones"].includes(serviceLineKey)) {
